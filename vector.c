...
static void vector_fail(vector_t vector, char *format, ...);
...
bool vector_reserve(const vector_t vector, const size_t capacity) {
    assert(vector);
    if (vector->capacity < capacity) {
        void *new_data = realloc(vector->member_size * capacity);
        if (!new_data) {
...
size_t vector_get(const vector_t *vector, const size_t index) {
}
